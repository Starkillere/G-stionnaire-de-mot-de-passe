...
def modifier(platforme:str, nom:str, database='databaose.db', master_key=None, new_platforme=None, new_nom=None, new_password=None, new_mail=None) -> bool :
    assert type(platforme) == str
    assert type(nom) == str
    assert type(new_mail) == str or new_mail == None
    assert type(new_platforme) == str or new_platforme == None
    ...
    with sqlite3.connect(database) as connection:
        cursor = connection.cursor()
        if new_platforme != None:
            requete = f"UPDATE gestionnaire SET platforme = '{new_platforme}' WHERE nom = '{nom}' and platforme = '{platforme}'"
        if new_nom != None:
        ...
        if new_mail != None:
            requete = f"UPDATE gestionnaire SET mail = '{new_mail}' WHERE nom = '{nom}' and platforme = '{platforme}'"
        cursor.execute(requete)
        connection.commit()
    return True
