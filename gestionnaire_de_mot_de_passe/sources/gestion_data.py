...
def modifier(platforme:str, nom:str, id:int, database='database.db', master_key=None, new_nom=None, new_password=None) -> bool :
    assert type(id) == int
    assert type(platforme) == str
    assert type(nom) == str
    ...
    with sqlite3.connect(database) as connection:
        cursor = connection.cursor()
        if new_nom != None:
            requete = f"UPDATE gestionnaire SET nom = '{new_nom}' WHERE id = {str(id)} AND nom = '{nom}' AND platforme = '{platforme}'"
        if new_password != None:
            my_pass = CodageMawa(master_key)
            requete = f"UPDATE gestionnaire SET password = '{my_pass.encode(new_password)}' WHERE id = {str(id)} AND nom = '{nom}' AND platforme = '{platforme}'"
        cursor.execute(requete)
        connection.commit()
    return True
