...
def save_password(platforme:str, nom:str, password:str,  master_key:str, mail=None, database='database.db') -> bool:
...
def lecture(return_all:bool, master_key:str, database='database.db', nom=None, platforme=None):
    assert type(master_key) == str
...
def importation(fichier_user:str, master_key:str, database='database.db') -> bool:
    ...
    if list(passwords[0].keys()) == ['platforme', 'nom', 'password', 'mail']:
        with sqlite3.connect(database) as connection:
            ...
            my_pass = CodageMoua(master_key)
            for password in passwords:
                requete = "insert into gestionnaire (platforme, nom, password, mail) values ('{platforme}', '{nom}', '{chiffre_password}', '{mail}')".format(platforme=password['platforme'], nom=password['nom'], chiffre_password=my_pass.encode(password['password']), mail=password['mail'])
                cursor.execute(requete)
                connection.commit()
    else:
       return False
...
def supprimer(delt_all:bool, platforme=None, nom=None, database='database.db') -> bool:
    assert type(delt_all) == bool
    assert type(database) == str and (database.split('.'))[-1] == 'db'
    if delt_all:
        with sqlite3.connect(database) as connection:
            cursor = connection.cursor()
            requete = "DELETE FROM gestionnaire"
            cursor.execute(requete)
            connection.commit()
    else:
        assert type(platforme) == str
...
def modifier(platforme:str, nom:str, database='database.db', master_key=None, new_nom=None, new_password=None) -> bool :
    assert type(platforme) == str
    assert type(nom) == str
