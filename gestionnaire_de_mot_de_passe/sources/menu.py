...
"background-color: rgb(255, 255, 255);")
    ...
    def confirmer(self):
        ...
        if len(mail) == 0:
            gestion_data.save_password(user['id'], platforme, nom, password, user['master key'])
            self.affichage.append("Successfull")
        else:
            gestion_data.save_password(user['id'], platforme, nom, password, user['master key'], mail)
            self.affichage.append("Successfull")
...
"background-color: rgb(255, 255, 255);")
    ...
    def confirmer(self):
        ...
        platforme = self.lineEdit_platforme.text()    
        nom = self.lineEdit_nom.text()
        com = gestion_data.lecture(False, user['master_key'], user['id'], nom=nom, platforme=platforme)
        if com == False:
...
"background-color: rgb(255, 255, 255);")
    ...
    def confirmer(self):
        ...
        if gestion_data.importation(fichier, user['master_key'], user['id']):
            self.affichage.append("Successfull") 
        else:
...
"background-color: rgb(255, 255, 255);")
    ...
    def confirmer(self):
        ...
        if len(new_password) != 0:
            gestion_data.modifier(platforme, nom, user['id'], master_key=user['master_key'], new_password=new_password) 
        if len(new_nom) != 0:
            gestion_data.modifier(platforme, nom, user['id'], master_key=user['master_key'], new_nom=new_nom)
        self.affichage.append("Successfull")
